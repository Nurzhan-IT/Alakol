...
class SearchListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Оптимизация: загружаем все удобства и типы номеров одним запросом и кэшируем результаты
        if not hasattr(self, '_all_features'):
            self._all_features = list(HotelFeatures.objects.values_list('name', flat=True).distinct())
        if not hasattr(self, '_room_types'):
            self._room_types = list(RoomType.objects.values_list('type', flat=True).distinct())
            
        context['all_features'] = self._all_features
        context['room_types'] = self._room_types
